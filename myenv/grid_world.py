...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, n_width: int = 10,
        ...
        self.randomEnds = False # 终止格子是否随机
        self.potentialEnds = [] # 可能的终止格子位置
        self.winRewards = 1
    ...
    def step(self, action):
        ...
        # notChanged = (new_x == old_x and new_y == old_y)
        self.reward = self.grids.get_reward(new_x, new_y) #+ notChanged*self.default_reward/2
    ...
    def reset(self):
        ...
        if self.randomEnds:
            self.viewer = None
            num = len(self.potentialEnds)
            idx = self.np_random.choice(np.arange(num),1,p=[1/num]*num)[0]
            self.ends = [self.potentialEnds[idx]]
            for i,(x,y) in enumerate(self.potentialEnds):
                self.grids.set_reward(x,y,self.default_reward)
            self.rewards = [ self.ends[0] + (self.winRewards, ) ]
            self.refresh_setting()
...
def MyRandomGridWorld0():
    '''无风11*11的格子，
    '''
    n_width=31
    n_height=31
    env = GridWorldEnv(n_width=31,
                       n_height=31,
                       action_num=4,
                       u_size=90,
                       default_reward=-1,
    ...
    env.start = ((n_width-1)//2,(n_width-1)//2)
    env.randomEnds = True
    env.potentialEnds = [(n_width-1, n_height-1),(0,0)]
    env.winRewards = 100
    # env.reset()
    # env.rewards = [(10, 10, 100),(0, 0, 100)]
    # env.types = [(7, 10, 1), (7, 7, 1), (10, 6, 1)]
    env.refresh_setting()
    return env

def MyRandomGridWorld1(seed):
    '''无风11*11的格子，
    '''
    n_width=15
    n_height=15
    env = GridWorldEnv(n_width=n_width,
                       n_height=n_height,
                       action_num=4,
                       u_size=90,
                       default_reward=-0.01,
    ...
    env.start = (3,3)
    env.randomEnds = True
    print("env.default_reward:",env.default_reward)
    env.potentialEnds = [(n_width-1, n_height-1),(0,0)]
    env.winRewards = 1
...
def MyRandomGridWorld2(seed):
    ...
    env.start = (3,3)
    env.randomEnds = True
    env.potentialEnds = [(0,0)]
    env.winRewards = 1
    env.types = [
   
         (7, 7, 1),(8, 6, 1) ,(7, 10, 1), (8, 9, 1), (10, 6, 1),(12,13,1),(13,12,1),
            (0,14,1),(1,12,1),(2,13,1),
            (14,0,1),(12,1,1),(13,2,1),
    ]
    return env

def MyRandomGridWorld3(seed):
    '''无风11*11的格子，
    '''
    n_width=15
    n_height=15
    ...
    env.start = (3, 3)
    env.randomEnds = True
    env.potentialEnds = [(n_width-1, n_height-1)] 
    env.winRewards = 1
    env.types = [
         (7, 7, 1),(8, 6, 1) ,(7, 10, 1), (8, 9, 1), (10, 6, 1),(12,13,1),(13,12,1),
            (0,14,1),(1,12,1),(2,13,1),
...
if __name__ == "__main__":
    ...
    #                    {'x': n_width-1, 'y': n_height // 2, 'hp': 100, 'damage': 20, 'attack_range': 1, 'max_hp': 100},
    #                    {'x': n_width-1, 'y': n_height // 2, 'hp': 100, 'damage': 20, 'attack_range': 3, 'max_hp': 100},
    #                ],
    #                # player_attribute_dict={'x': 11 // 2, 'y': 11 // 2, 'hp': 80, 'damage': 20, 'attack_range': 1, 'max_hp': 100,
    #                #                        'heal_buff': 1}
    #                player_attribute_dict={'x': 0, 'y': 0, 'hp': 80, 'damage': 20, 'attack_range': 1,
    #                                       'max_hp': 100,
    #                                       'heal_buff': 1},
    #                damage_reward_coef=1.,
    #                heal_reward_coef=1.,
